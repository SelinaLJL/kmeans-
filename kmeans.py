...
def jingweiDistance(vector1,vector2):
    R=6371.0
    X1=vector1[0]
    Y1=vector1[1]
    X2=vector2[0]
    Y2=vector2[1]
    return R * arccos(cos(Y1) * cos(Y2) * cos(X1 - X2) + sin(Y1) *sin(Y2))
    ...
    while clusterChanged:
        ...
        for i in range(numSamples):  # range
            ...
            for j in range(k):
                ...
                #对于经纬度，用经纬度求的的直线距离分类
                distance = jingweiDistance(centroids[j, :], dataSet[i, :])
                if distance < minDist:
                    minDist = distance
                    minIndex = j
...
def showCluster(dataSet, k, centroids, clusterAssment):
    ...
    for i in range(numSamples):
        ...
        if(i<5):
            plt.text(dataSet[i, 0]+1, dataSet[i, 1],"start"+str(i+1))
